...
import ZonesTab from '../components/master-data/ZonesTab';

type Tab = 'zones' | 'facilities' | 'vendors' | 'customers' | 'services';
  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ padding: '16px 16px 0' }}>
        ...
        <p style={{ fontSize: 11, color: '#9ca3af', margin: '2px 0 0' }}>Manage zones, facilities, vendors, customers, and services</p>
      </div>

      {/* Tabs */}
      <div style={{ padding: '12px 16px 0', display: 'flex', gap: 2 }}>
        {([['zones', 'Zones'], ['facilities', 'Facilities'], ['vendors', 'Vendors'], ['customers', 'Customers'], ['services', 'Services']] as const).map(([key, label]) => (
      ...
      <div style={{ padding: 16 }}>
        {tab === 'zones' && <ZonesTab />}
        {tab === 'facilities' && <LocationsTab onNavigateToZones={() => setTab('zones')} />}
        {tab === 'services' && <ServicesTab />}
        {tab !== 'zones' && tab !== 'facilities' && tab !== 'services' && (
