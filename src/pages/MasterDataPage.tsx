...
import RegionsTab from '../components/master-data/RegionsTab';

type Tab = 'facilities' | 'regions' | 'vendors' | 'customers' | 'services';
  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ padding: '16px 16px 0' }}>
        ...
        <p style={{ fontSize: 11, color: '#9ca3af', margin: '2px 0 0' }}>Manage facilities, regions, vendors, customers, and services</p>
      </div>

      {/* Tabs */}
      <div style={{ padding: '12px 16px 0', display: 'flex', gap: 2 }}>
        {([['facilities', 'Facilities'], ['regions', 'Regions'], ['vendors', 'Vendors'], ['customers', 'Customers'], ['services', 'Services']] as const).map(([key, label]) => (
      ...
      <div style={{ padding: 16 }}>
        {tab === 'facilities' && <LocationsTab />}
        {tab === 'regions' && <RegionsTab />}
        {tab === 'services' && <ServicesTab />}
        {tab !== 'facilities' && tab !== 'regions' && tab !== 'services' && (
