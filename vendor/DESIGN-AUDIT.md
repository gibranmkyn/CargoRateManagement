...
> Last updated: 2026-09-02 (third audit pass — verified fix status of prior findings, added MINOR-04)

## Summary

The vendor app implementation is largely compliant with the design system. No major color, shadow, or radius violations were found. MINOR-01 and MINOR-02 from the Aug-25 audit were fixed. Two violations remain outstanding (VIOLATION-01 and VIOLATION-02). One new minor issue added (MINOR-04).

---

## Findings

### ~~MINOR-01~~ ✅ FIXED — Dead `borderTop` declaration in `renderDriverVehicle`

**File:** `vendor/src/pages/JobDetailPage.tsx`  
**Status:** Fixed — `borderTop` declaration no longer present; `renderDriverVehicle` div uses only `border: '1px solid rgba(21,44,255,0.1)'`.

---

### ~~MINOR-02~~ ✅ FIXED — Activity log count badge uses `borderRadius: 99`

**File:** `vendor/src/pages/JobDetailPage.tsx`  
**Status:** Fixed — badge now uses `borderRadius: 6`. ✓
...
### MINOR-04 — `fontStyle: 'italic'` on "No driver assigned" text

**File:** `vendor/src/pages/JobDetailPage.tsx`, `renderDriverVehicle()` (approx. line 328)  
**Severity:** Minor
...
Italic is not part of the design system's approved typography. All muted/placeholder text uses regular weight at `#9ca3af` or `#d1d5db`. Italic introduces a visual style not present anywhere else in either app.

**Fix:** Remove `fontStyle: 'italic'`. Use `color: '#d1d5db'` (ghost gray) to match the existing "No driver assigned" sub-line in the job list table.

```jsx
// Fixed
<div style={{ fontSize: 11, color: '#d1d5db' }}>No driver assigned</div>
```
...
**Status:** Outstanding — see HMW-V14 for design decision (open question: urgency coloring threshold). Once HMW-V14 is resolved, implement Option B.
...
Current implementation uses a small `+ Add` ghost button in the section header that triggers the hidden file input. The upload zone is only accessible if the vendor finds the small button or uses the Status Action Bar upload button. On tablet, the camera capture path is not discoverable.
...
## Compliant Areas (checked — 2026-09-02)
...
| Activity log count badge: borderRadius 6 (not 99) | ✓ Fixed |
| renderDriverVehicle: single border shorthand (no dead borderTop) | ✓ Fixed |

---

## Open HMWs gating implementation

| Violation | Gating HMW | Status |
|-----------|-----------|--------|
| VIOLATION-01 (FM timeline times) | HMW-V14 | Open — urgency coloring threshold needs user input |
| HMW-V15 (quantity reconciliation) | HMW-V15 | New — needs user input on Option A vs B and post-verification flagging |
